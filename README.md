...
## **Hardware**
   - If you have **ANY** different hardware than what's listed in the hardware table below, make sure to change Kexts, SSDTs, Drivers, add or remove patches and update your config.plist accordingly, see: **"What if I'm not familiar with the ACPI/SSDT files in the EFI?"**
...
- No sound plays when connecting a charger (PowerChime) by default. To fix this, open Terminal and type: `defaults write com.apple.PowerChime ChimeOnAllHardware -bool true; open /System/Library/CoreServices/PowerChime.app &`.
...
| "What if I'm not familiar with the ACPI/SSDT files in the EFI?" | Check the *EFI Files Explained* section below. |
